...
class Optim:
    ...
    def sys_errors(self):
        ...
        Returns:
            self.sys_errors: an array containing [total error, non-linearity, stray light, crosstalk, flat-field, bad pixel, keystone/smile, memory, striping] estimates.

        '''
        self.nonlinearity = self.cfg.nonlinearity
        self.stray_light = np.sqrt((self.cfg.fo_reflectivity)**2 + (self.cfg.lens_reflectivity)**2 + (self.cfg.mirror_reflectivity)**2 + (self.cfg.ar_coatings)**2 + (self.cfg.leakage)**2 + (self.cfg.ghosting)**2)
        ...
        self.sys_error = np.sqrt((self.nonlinearity)**2 + (self.stray_light)**2 + (self.ffr)**2 + (self.bad_pixels)**2 + (self.crosstalk)**2 + (self.key_smile)**2 + (self.striping)**2 + (self.memory)**2)

        self.sys_errors = [self.sys_error, self.nonlinearity, self.stray_light, self.crosstalk, self.ffr, self.bad_pixels, self.crosstalk, self.key_smile, self.striping, self.memory]
    ...
    def rand_errors(self):
        ...
        Returns:
            self.rand_errors: an array containing [total error, dark current, readout, quantization, photon noise] estimates.
