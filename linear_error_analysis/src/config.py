...
def parse_config():
    ...
    parser.add_argument(
        "--spectral_lower", type=float, default=1590, help="wavelength (nm)"
    )
    parser.add_argument(
        "--spectral_upper", type=float, default=1680, help="wavelength (nm)"
    )
    parser.add_argument(
        "--fwhm",
        type=float,
    ...
    parser.add_argument("--sza", type=float, default=50.0, help="solar zenith angle")
    parser.add_argument("--vza", type=float, default=0.0, help="viewing zenith angle")
    ...
    # Pixel Crosstalk
    parser.add_argument(
        "--crosstalk", type=float, default=0.0039, help="crosstalk between pixels"
    )
    # Stray Light
    parser.add_argument(
        "--fo_reflectivity",
    ...
    parser.add_argument(
        "--leakage", type=float, default=0.10, help="stray light leakage"
    )
    parser.add_argument(
        "--ghosting",
        type=float,
    ...
    # Keystone and Smile
    parser.add_argument(
        "--keystone", type=float, default=0.01, help="keystone distortion error"
    )
    parser.add_argument(
        "--smile", type=float, default=0.01, help="smile distortion error"
    )
    # Striping
    parser.add_argument(
        "--striping", type=float, default=0.05, help="striping distortion error"
    )
    # Memory
    parser.add_argument(
        "--memory", type=float, default=0.02, help="memory effect pixel error"
    )

    ### Random Error Quantification
    parser.add_argument(
        "--x_pixels", type=int, default=512, help="number of pixels in the X direction"
    )
    parser.add_argument(
        "--y_pixels", type=int, default=640, help="number of pixels in the Y direction"
    )
    parser.add_argument(
